			...
			if(process.getType()==1){
				...
				while(process.getPageNo()<=totalPageNo && process.getFinished()==0){
					...
					while (iurl.hasNext()){
						...
						logger.info("tempHouseIndex null:"+houseindex);
						if(tempHouseIndex!=null){
							logger.info("tempHouseIndex :"+JSONObject.toJSONString(houseindex));
							if(tempHouseIndex.getStatus()!=null&&tempHouseIndex.getStatus()==1)
								continue;
							else{
								houseindex.setStatus(1);//设置为在售
								houseindex.setUpdatetime(new Date());
								logger.info("houseindexService null:"+houseindexService);
